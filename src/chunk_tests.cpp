                      ...
                      host_workspace<P> const &host_space)
{
  ...
  fk::vector<P> const input_copy(rank_space.batch_input);
                   ...
                   int const num_ranks)
{
  ...
  for (auto const &[rank, grid] : plan)
  {
    ...
    std::mt19937 mersenne_engine(rd());
    std::uniform_real_distribution<P> dist(-2.0, 2.0);
    fk::vector<P> batch_out_h(rank_space.batch_output);
    auto gen = [&dist, &mersenne_engine]() { return dist(mersenne_engine); };
                      ...
                      rank_workspace<P> const &rank_space)
{
  ...
  for (auto const &[row, cols] : chunk)
  {
    ...
    fk::matrix<P> reduction_copy(reduction_matrix);
  ...
  auto const abs_compare   = [](double const a, double const b) {
                    ...
                    int const num_ranks)
{
  ...
  for (auto const &[rank, grid] : plan)
  {
    ...
    fk::vector<P> reduction_h(rank_space.reduction_space);
    auto gen = [&dist, &mersenne_engine]() { return dist(mersenne_engine); };
