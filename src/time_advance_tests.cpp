                       ...
                       std::vector<std::string> const &additional_args = {})
{
  ...
  for (int i = 0; i < pde.num_dims; ++i)
  {
    for (int j = 0; j < pde.num_terms; ++j)
    {
      ...
      fk::matrix<P> coeffs =
          fk::matrix<P>(generate_coefficients(dim, term, init_time));
                   ...
                   "[time_advance,fokker]", float, double)
{
  ...
  SECTION("fokkerplanck_1d_4p2, level 2, degree 2, full grid")
  {
    int const degree = 2;
    int const level  = 2;
    std::string const gold_base =
        "../testing/generated-inputs/time_advance/fokkerplanck1_4p2_fg_l2_d2_t";
    auto pde = make_PDE<TestType>(PDE_opts::fokkerplanck_1d_4p2, level, degree);
    bool const full_grid = true;
    time_advance_test(level, degree, *pde, num_steps, gold_base, full_grid);
  }
